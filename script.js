...
function createHexagon(svg, startX, startY, size, offsetX, offsetY) {
    ...
    centerPolyAngles.forEach(angle => {
        ...
        const CenterPolyPoints = {x1: startX + (size /2) + offsetX, y1: startY + offsetY + Math.sqrt(3) / 2 * (size /3),x2: startX + (size * (1- 1/3)) + offsetX, y2: startY + offsetY + Math.sqrt(3) / 2 * (size /3),x3: centerX, y3: centerY - (size /3)}
